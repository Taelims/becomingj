...
import MakeCalendar from "../modules/MakeCalendar";
...
const Calendar = () => {
  ...
  // 일정 입력
  const onConfirm = ({ index, todo, todos }) => {
    if (todos.length !== 0) {
      todos.map((item) => {
        dispatch({ type: "INSERT", index: item, todo: todo });
      });
    } else {
      dispatch({ type: "INSERT", index: index, todo: todo });
    }
    dispatch({ type: "MODAL" });
  };
  // 일정 입력 취소
